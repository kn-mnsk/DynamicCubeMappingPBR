...
#include <OGLDEV_local/picking_technique.h>
...
bool PickingTechnique::Init()
{
    ...
    if (!AddShader(GL_VERTEX_SHADER, get_shader_root("OGLDEVTut29_picking.vert").c_str())) {
        return false;
    }

    if (!AddShader(GL_FRAGMENT_SHADER, get_shader_root("OGLDEVTut29_picking.frag").c_str())) {
        return false;
    }

...
#include <LearnOpenGL/stb_helper.h>
...
#include <custom/learnopengl/mycamera.h>
...
void genMeshSphere(GLuint x_segs, GLuint y_segs, meshVertices& trianglestrip, meshVertices& patches, float sphereRadius , bool debug);
void genMeshTorus(GLfloat R, GLuint R_segs, GLfloat r, GLuint r_segs, meshVertices& trianglestrip, meshVertices& patches, bool debug);
void genMeshParaboloid(GLfloat semimajor, GLfloat semiminor, GLuint elliptic_segs, GLfloat height, GLuint height_segs, meshVertices& trianglestrip, meshVertices& patches, bool debug);
void genMeshCube(meshVertices& patches, LineVertices& normalline, float bendnormal, bool debug);
void genMeshQuadXZ(meshVertices& patches);
void genMeshQuadXY(meshVertices& patches);
void genMeshQuadXY2(meshVertices& patches, float quadside, int segs, bool debug);
...
void renderCubeMap(GLuint vao, GLuint vbo);
...
void writeCubemap_png(GLuint cubemapID, int width, int height, std::string filename);
void writeCubemap_hdr(GLuint cubemapID, int width, int height, std::string filename);
void writeTexture2D_bmp(GLuint textureID, int width, int height, std::string filename);
...
void genMeshSphere(GLuint x_segs, GLuint y_segs, meshVertices& trianglestrip, meshVertices& patches, float sphereRadius = 1.0f, bool debug = false) {
	...
	if (debug) { // DEBUG::output vertices and indices of trianglestrips
		...
		trianglesIndices_file.open(get_localResources("other/spherePrianglesIdices.txt"));
		...
		trianglesPos_file.open(get_localResources("other/spherePrianglesPosition.txt"));
	...
	if (debug) { // DEBUG::output vertices and indices of patches
		...
		patchesIndices_file.open(get_localResources("other/spherePatchesIdices.txt"));
		...
		patchesPos_file.open(get_localResources("other/spherePatchesPosition.txt"));
...
void genMeshTorus(GLfloat R, GLuint R_segs, GLfloat r, GLuint r_segs, meshVertices& trianglestrip, meshVertices& patches, bool debug = false) {
	...
	if (debug) { // output vertices and indices of trianglestrips
		std::ofstream trianglesindices_file;
		trianglesindices_file.open(get_localResources("other/torusPrianglesidices.txt"));
		...
		std::ofstream trianglespos_file;
		trianglespos_file.open(get_localResources("other/torusPrianglesposition.txt"));
...
void genMeshParaboloid(GLfloat semimajor, GLfloat semiminor, GLuint elliptic_segs, GLfloat height, GLuint height_segs, meshVertices& trianglestrip, meshVertices& patches, bool debug = false) {
	...
	if (debug) { // output vertices and indices of trianglestrips
		std::ofstream trianglesIndices_file;
		trianglesIndices_file.open(get_localResources("other/paraboloidTrianglesIdices.txt"));
		for (unsigned int i = 0; i < trianglestrip.indices.size(); i++) {
			trianglesIndices_file << trianglestrip.indices[i] << "\n";
		}
		trianglesIndices_file.close();
		...
		std::ofstream trianglesPos_file;
		trianglesPos_file.open(get_localResources("other/paraboloidPrianglesPosition.txt"));
		for (unsigned int i = 0; i < trianglestrip.vertices.size(); i++) {
			trianglesPos_file << trianglestrip.vertices[i].position.x << "," << trianglestrip.vertices[i].position.y << "," << trianglestrip.vertices[i].position.z << "\n";
		}
		trianglesPos_file.close();
	...
	if (debug) { // output vertices and indices of patches
		std::ofstream patchesIndices_file;
		patchesIndices_file.open(get_localResources("other/paraboloidPatchesIdices.txt"));
		for (int i = 0; i < patches.indices.size(); i++) {
			patchesIndices_file << patches.indices[i] << "\n";
		}
		patchesIndices_file.close();
		...
		std::ofstream patchesPos_file;
		patchesPos_file.open(get_localResources("other/paraboloidPatchesPosition.txt"));
		for (int i = 0; i < patches.vertices.size(); i++) {
			patchesPos_file << patches.vertices[i].position.x << "," << patches.vertices[i].position.y << "," << patches.vertices[i].position.z << "\n";
		}
		patchesPos_file.close();
...
void genMeshCube(meshVertices& patches, LineVertices& normalline, float bendnormal = 0.0f, bool debug = false) { // bendnormal in degree
	...
	if (debug) { // output vertices and indices of patches
		std::ofstream patchesIndices_file;
		patchesIndices_file.open(get_localResources("other/cubePatchesIdices.txt"));
		for (int i = 0; i < patches.indices.size(); i++) {
			patchesIndices_file << patches.indices[i] << "\n";
		}
		patchesIndices_file.close();
		...
		std::ofstream patchesPos_file;
		patchesPos_file.open(get_localResources("other/cubePatchesPosition.txt"));
		for (int i = 0; i < patches.vertices.size(); i++) {
			patchesPos_file << patches.vertices[i].position.x << "," << patches.vertices[i].position.y << "," << patches.vertices[i].position.z << "\n";
		}
		patchesPos_file.close();
...
void genMeshQuadXY2(meshVertices& patches, float quadside = 2.0f, int segs = 2, bool debug = false) {
	...
	if (debug) { // output vertices and indices of patches
		std::ofstream patchesIndices_file;
		patchesIndices_file.open(get_localResources("other/quadPatchesIdices.txt"));
		for (int i = 0; i < patches.indices.size(); i++) {
			patchesIndices_file << patches.indices[i] << "\n";
		}
		patchesIndices_file.close();
		...
		std::ofstream patchesPos_file;
		patchesPos_file.open(get_localResources("other/quadPatchesPosition.txt"));
		for (int i = 0; i < patches.vertices.size(); i++) {
			patchesPos_file << patches.vertices[i].position.x << "," << patches.vertices[i].position.y << "," << patches.vertices[i].position.z << "\n";
		}
		patchesPos_file.close();
...
void drawElements(GLuint vao, GLuint vbo, GLuint ebo, GLenum mode, GLuint indicessize, const void* indices) {
	...
	case GL_PATCHES: {
		...
		if (errNum != GL_NO_ERROR) {
			std::cout << "glDrawElements Error: " << errNum << std::endl;
		}
...
void writeCubemap_png(GLuint cubemapID, int width, int height, std::string filename) {
	...
	for (unsigned int j = 0; j < 6; ++j) {
		...
		stbi_write_png(get_localResources(filename + std::to_string(j) + ".png").c_str(), width, height, 4, BufOutPtr, width * 4);
		free(BufOutPtr);
	}
...
void writeCubemap_hdr(GLuint cubemapID, int width, int height, std::string filename) {
	...
	for (unsigned int j = 0; j < 6; ++j) {
		...
		glGetTexImage(GL_TEXTURE_CUBE_MAP_POSITIVE_X + j, 0, GL_RGB, GL_FLOAT, BufOutFloatPtr);
		stbi_flip_vertically_on_write(false);
		stbi_write_hdr(get_localResources(filename + std::to_string(j) + ".hdr").c_str(), 512, 512, 3, BufOutFloatPtr);
		free(BufOutFloatPtr);
	}
	glBindTexture(GL_TEXTURE_CUBE_MAP, 0);
...
void writeTexture2D_bmp(GLuint textureID, int width, int height, std::string filename) {
	...
	stbi_write_bmp(get_localResources(filename + ".bmp").c_str(), width, height, 4, BufOutPtr);
	free(BufOutPtr);
	glBindTexture(GL_TEXTURE_2D, 0);

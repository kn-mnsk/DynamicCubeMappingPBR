...
namespace SolarSys {
	...
	void genShaderProgram(std::string filename) {
		...
		for (std::vector<std::string> s : data) {
			ShaderProgram element{
				...
				get_shader(s[2]),
				get_shader(s[3]),
				s[4] == "NULL" ? s[4] : get_shader(s[4]),
				s[5] == "NULL" ? s[5] : get_shader(s[5]),
				s[6] == "NULL" ? s[6] : get_shader(s[6]),
				nullptr,
				{},
				{},
				{},
	...
	void genTextureObject(std::string filename) {
		...
		for (std::vector<std::string> t : data) {
			TextureData element{
				t[0],
				(t[1] == "NULL") ? "NULL" : get_commonResources(t[1]),
				(GLuint)std::stoi(t[2]),
				t[3],
	...
	public:
		...
		void updateMotionData(bool debug) {
			...
			if (debug == true) {
				...
				debugWriteMotionData(get_localResources("other/solarsys_debug_motiondata.csv"));
				// Debug End
